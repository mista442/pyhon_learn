...
x,y = InputData1() # записали в глобальные переменные x,y значения из локальных переменных функкции InputData1()
def Summa(n1,n2): # обьявляем аргументы
    s=n1+n2
    return s

sum=Summa(x,y) # подставляем в аргументы указанные переменныей
