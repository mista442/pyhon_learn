...
# foo6(a,d,e) # 1+4+5 = 10
# #foo6(a,arg2=10,c) # SyntaxError: positional argument follows keyword argument. Позиционные аргументы после именованных вызывать нельзя. Даже если при описании функции этот аргумент описан как именованный.
...
# tup=(1,2,3,4) # кортеж который будет подставляться под *args
#
# #foo8(*tup,b=35) # TypeError: foo8() got multiple values for argument 'b'. Аргумент b не может быть сразу принимать два значения - 2 и 35.
# foo8(*tup,c=35) # 1 2 (3,4) 35.
# foo8(1,5,6,*tup,c=35) # 1 5 (6, 1, 2, 3, 4) 35. Число 6 попадает в кортеж, т.к. для него не описан никакой аргумент при задании функции. И оно вставляется в начало кортежа
...
# kwargs принимает аргументы в виде ключа и значения, которые потом преобразуются в словарь - key:value.

# def foo9(**kwargs):
#     print(kwargs)
#
# foo9(a=1,b='test',c=16.1) # {'a': 1, 'b': 'test', 'c': 16.1}
# foo9(a=1,b=2,c=3) # {'a': 1, 'b': 2, 'c': 3}
# foo9(b=20,v='all',c=3) # {'b': 20, 'v': 'all', 'c': 3}

#------------------
# def foo10(*args,**kwargs):
#     print(args,kwargs)
#
# foo10(a=1,b='test',c=16.1) # {'a': 1, 'b': 'test', 'c': 16.1} # если данные переданы в формате только ключ:значение, то в кортеж ничего не будет записано, а кортеж отобразится пустой
# foo10(10,'hello',a=1,b=2,c=3) # {'a': 1, 'b': 2, 'c': 3} # 10, hello переданы в качестве обычных элементов и будут записаны в кортеж (args)
# foo10(1,2,b=20,v='all',c=3) # {'b': 20, 'v': 'all', 'c': 3}
