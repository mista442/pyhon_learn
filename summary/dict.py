...
# НО get работает по другому на самом деле. Если нужно получить значение ключа, чтобы дальше его где-то использовать, то нужно использовать именно метод get! print выводит только сообщение, но не передает куда-то дальше показанное значение.
